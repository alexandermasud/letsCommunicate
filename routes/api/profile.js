...
router.get("/me", auth, async (req, res) => {
  try {
    ...
    if (!profile) {
      return res
        .status(400)
        .json({ msg: "Ingen profil för användaren hittad" });
    }
...
router.post(
  ...
  [
    auth,
    check("status", "Vänligen ange status")
      .not()
      .isEmpty()
  ],
  async (req, res) => {
    ...
    try {
      ...
      // If profile not found we create a new profile
...
router.delete("/", auth, async (req, res) => {
  try {
    ...
    res.json({ msg: "Användare borttagen" });
